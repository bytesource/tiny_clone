...
%w(rubygems sinatra data_mapper haml uri rest-client xmlsimple ./dirty_words).each do |lib|
  require lib
end
...
# It is used to create the identifier URL.
...
# The identifier URL route is the one that is most frequently used.
# Given the identifier URL, it redirects the user to the original URL.
...
class Link
  ...
  def self.create_link(original)
    # It seems a primary key id is allocated for each instance 
    # (which gets discarded even if the url is not saved to the database.)
    
    # NOTE:     ::create saves the Url instance immediately, but we don't want that as it might
    #           get discarded. 
    # Solution: Use ::new, save later using ::save if needed.
    url = Url.create(:original => original) 
    # to_s(base=10) → string
    # Returns a string containing the representation of fix radix base (between 2 and 36)
    # 12345.to_s(36)   #=> "9ix
    identifier_link = url.id.to_i.to_s(36) # http://stackoverflow.com/questions/6727490/how-do-i-handle-the-wrong-number-of-method-arguments
    # We only proceed if the shortened link is not found in the links table and does not contain any dirty words.
    if Link.first(:identifier => identifier_link).nil? and !DIRTY_WORDS.include?(identifier_link) # before: 'or'
      link = Link.new(:identifier => identifier_link)
      link.url = url
      link.save
      return link
...
class Visit
  ...
  def self.count_by_date_with(identifier, num_of_days)
    ...
    # Chooses those dates that are associated with the correct identifier (identifier link) and that
